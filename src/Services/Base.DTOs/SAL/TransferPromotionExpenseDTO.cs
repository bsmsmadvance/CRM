...
namespace Base.DTOs.SAL
{
    public class TransferPromotionExpenseDTO : BaseDTO
    {
        ...
        public async static Task<TransferPromotionExpenseDTO> CreateFromModelAsync(TransferPromotionExpense model, Guid unitPriceID, DatabaseContext db)
        {
            if (model != null)
            {
                ...
                var item = await db.UnitPriceItems.Include(o => o.PriceUnit).Where(o => o.UnitPriceID == unitPriceID && o.MasterPriceItemID == model.MasterPriceItemID).FirstOrDefaultAsync();
        ...
        public async static Task<List<TransferPromotionExpenseDTO>> CreateDraftFromUnitAsync(Guid agreementId, DatabaseContext db)
        {
            ...
            var priceList = await db.PriceLists.GetActivePriceListAsync(unit.ID);
