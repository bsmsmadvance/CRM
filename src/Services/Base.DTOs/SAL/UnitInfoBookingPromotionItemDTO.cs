...
{
    public class UnitInfoBookingPromotionItemDTO : BaseDTO
    {
        /// <summary>
        /// ID ของ MasterBookingPromotionItem
        /// </summary>
        public Guid? FromMasterBookingPromotionItemID { get; set; }
        /// <summary>
        /// ID ของ QuotationBookingPromotionItem
        /// </summary>
        ...
        /// รายการย่อย
        /// </summary>
        public List<UnitInfoBookingPromotionItemDTO> SubItems { get; set; }
        /// <summary>
        /// รายการที่ถูกเลือก
        /// </summary>
        public bool IsSelected { get; set; }
        public static UnitInfoBookingPromotionItemDTO CreateFromMasterModel(MasterBookingPromotionItem itemModel, MasterBookingPromotionFreeItem freeItemModel, MasterBookingCreditCardItem creditModel, DatabaseContext db)
        {
            UnitInfoBookingPromotionItemDTO result = new UnitInfoBookingPromotionItemDTO();
            if (itemModel != null)
            {
                ...
                result.SubItems = new List<UnitInfoBookingPromotionItemDTO>();
                if (subItems.Count > 0)
                {
                    foreach (var item in subItems)
                    {
                        var promoItem = new UnitInfoBookingPromotionItemDTO()
                        {
        ...
        public static UnitInfoBookingPromotionItemDTO CreateFromModel(BookingPromotionItem itemModel, BookingPromotionFreeItem freeItemModel, BookingCreditCardItem creditModel, DatabaseContext db)
        {
            UnitInfoBookingPromotionItemDTO result = new UnitInfoBookingPromotionItemDTO();
            if (itemModel != null)
            {
