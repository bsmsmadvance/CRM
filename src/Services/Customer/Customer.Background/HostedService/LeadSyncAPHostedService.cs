...
{
    public class LeadSyncAPHostedService : IHostedService
    {
        public IServiceProvider Services { get; }
        private Timer _timer;

        public LeadSyncAPHostedService(IServiceProvider services)
        {
            Services = services;
        }
                        ...
                        using (var tran = db.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                var task = Task.Run(async () =>
                                {
                                    ...
                                    var data = await leadService.SyncLeadsFromAPThaiWeb(startTime, endTime);
