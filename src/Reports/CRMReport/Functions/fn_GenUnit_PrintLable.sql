...
BEGIN
	...
	DECLARE @v TABLE(nOrder INT IDENTITY,UnitNumber nvarchar(40));
	DECLARE @A TABLE(nOrder INT IDENTITY,ProductID nvarchar(40),UnitNumber nvarchar(40),ContactID nvarchar(40))
	DECLARE @B TABLE(nOrder INT IDENTITY,ProductID nvarchar(40),UnitNumber nvarchar(40),ContactID nvarchar(40))
    
	INSERT INTO @A
		Select	AG.ProjectID,U.UnitNo,AO.FromContactID
		From	[SAL].[Agreement] AG
				LEFT OUTER JOIN [SAL].[AgreementOwner] AO ON AO.AgreementID = AG.ID AND AO.IsMainOwner = 1 AND ISNULL(AG.IsDeleted,0) = 0
                LEFT OUTER JOIN [PRJ].[Unit] U ON U.ID = AG.UnitID
 		Where	AG.UnitID IN (SELECT * FROM dbo.fn_SplitString(Replace(@UnitNumber,'''',''),',')) AND AG.ProjectID = @ProductID
		Order by AO.FromContactID,U.UnitNo

	INSERT INTO @B
		Select	AG.ProjectID,U.UnitNo,AP.FromContactID
		From	[SAL].[AgreementOwner] AP
				LEFT OUTER JOIN [SAL].[Agreement] AG ON AG.ID = AP.AgreementID 
                LEFT OUTER JOIN [PRJ].[Project] P ON P.ID = AG.ProjectID
                LEFT OUTER JOIN [PRJ].[Unit] U ON U.ID = AG.UnitID
		Where AP.ContactNo = @ContactID And P.ProjectNo = @ProductID
		Order by AG.ProjectID, U.UnitNo
	
	INSERT INTO @V
	Select	B.UnitNumber
	From	@B B 
		INNER JOIN @A A ON A.ProductID = B.ProductID AND A.UnitNumber = B.UnitNumber AND A.ContactID = B.ContactID 

					
	-- ดึงเอารายการแรกมาเป็นค่าเริ่มต้นข้อความก่อน
	SET @result = (SELECT TOP 1 UnitNumber FROM @v);	
	...
	DELETE FROM @v WHERE UnitNumber=@result;	-- ลบรายการแรกที่ดึงเอาไปผสมข้อความแล้วทิ้ง

	-- ผสมข้อความ
	DECLARE @tmp nvarchar(200);
	WHILE EXISTS(SELECT * FROM @v)
	  BEGIN
		SET @tmp = (SELECT TOP 1 UnitNumber FROM @v);
		SET @result = @result+','+ @tmp;		
		DELETE FROM @v WHERE UnitNumber=@tmp;
	  END
