...
--select [dbo].NumberToWords('21.11')
--SELECT [dbo].[NumberToWords]('66.00')

CREATE FUNCTION [dbo].[NumberToWords] (
	@Input NUMERIC (38, 2) -- Input number with as many as 18 digits
...
IF (@Cents < 0)  Set @Number = @Number - 1
...
INSERT INTO @NumbersTable   SELECT '00', 'Oh'
...
BEGIN
    ...
    SET @outputString
        = (@outputString) + ' Point ' +
          (
              SELECT word FROM @NumbersTable WHERE '0' + @tensones = number
          ) + ' ' + CASE
                        WHEN @ones <> '0' THEN
                        (
