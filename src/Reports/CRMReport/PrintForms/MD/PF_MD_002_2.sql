...
--[dbo].[AP2SP_PF_AG_005_2] '10039AA00595'


CREATE PROC [dbo].[PF_MD_002_2]
...
BEGIN
	IF(@Period<>@StartPeriod)
		...
		BEGIN
			IF (@PayableAmount <> @StartMoney)
			  BEGIN 
				SELECT @EndDate = DueDate  FROM ICON_EntForms_AgreementPeriod WITH (NOLOCK)
				Where Period=(@Period-1) and PaymentType = '6' and ContractNumber = @ContractNumber
...
INSERT INTO @t
SELECT RIGHT('00'+convert(nvarchar(10),(SELECT MAX(Period)+1 FROM ICON_EntForms_AgreementPeriod WITH (NOLOCK) WHERE Period>0 and PaymentType = '6' and ContractNumber = @ContractNumber)),3),
...
SELECT * from @t
else select '9999'Period,0 AMT,'Test'AMTTtext,NULL DueDate */
