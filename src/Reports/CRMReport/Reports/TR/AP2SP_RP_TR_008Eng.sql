...
--[dbo].[AP2SP_RP_TR_008Eng] '','40011','''03A09''',''
ALTER PROC  [dbo].[AP2SP_RP_TR_008Eng]
...
SET @LandWa = (SELECT ISNULL(AreaFromPFB,ISNULL(AreaFromRE,0))
			...
			WHERE ProductID=@ProductID AND UnitNumber=@UnitNumber);
...
,'Project' = '' --p.ProjectNameEng
,'ProjectTel' = '' --Isnull(p.Tel,'') ProjectTel
...
,'LandOfficeName' = '' --vt.LandOfficeNameEng
...
,'BankName' = '' --Replace(Isnull(bank.BankNameEng,''),'โอนสด','')BankName
...
,'CompanyName' = '' --dbo.fn_GetCompanyNameEn(P.CompanyID, ISNULL(tf.TransferDate,GETDATE())) --,com.CompanyNameThai CompanyName
,'NitiName' = '' --p.BankProjectAPEng
...
Left Join (Select Sum(Amount)Amount,ReferentID ContractNumber
			From ICON_Payment_PaymentDetails pd
			Where pd.PaymentType In('4','5','6')
			Group By ReferentID) pdAgreement on pdAgreement.ContractNumber=a.ContractNumber
Left Join (Select Sum(Amount)Amount,ReferentID BookingNumber
			From ICON_Payment_PaymentDetails pd
			Where pd.PaymentType In('4')
			Group By ReferentID) pdBooking on pdBooking.BookingNumber=a.BookingNumber
Left Join (Select Sum(Amount)Amount,ReferentID ContractNumber
			From ICON_Payment_PaymentDetails pd
			Where pd.PaymentType In('8','a06')
			Group By ReferentID) pdTransfer on pdTransfer.ContractNumber=a.ContractNumber
...
Order by a.ProductID,a.UnitNumber ASC,CB.Isselected DESC,Case When CB.Status = 1 Then 7 When CB.Status = 3 Then 8 When CB.Status = 2 Then 9 When CB.Status = 4 Then 10 Else 0 End Asc,CB.LoanDate Asc,Case when dcl.Status = 1 Then 1 Else 0 End Desc,CB.ID Asc
*/
