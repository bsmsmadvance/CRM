...
--  [AP2SP_NRP_TR_004_CD]NULL,'10025','2009-10-17','2013-10-21','Administrator Account'
ALTER PROC [dbo].[AP2SP_NRP_TR_004_CD]
...
SELECT ''TransferNumber'' = '''' --T.TransferNumber
	...
	,''Total'' = '''' --(T.IncomeTax+T.LocalTax+T.BusinessTax+ISNULL(TP.Transferfeetotalamount,0))-T.AllIncomeTax,
	...
	,''ValueMortgage'' = '''' --TP.ValueMortgage,
	,''WbsNumber'' = '''' --REPLACE(REPLACE(U.wbsnumber,''CO-0'',''CO-P''),''-R-'',''-P-'')

FROM [SAL].[Booking] B' --This is temp table. Actutal table start from below 
    /* [ICON_EntForms_Transfer]T 
	 LEFT OUTER JOIN [ICON_EntForms_Agreement]A ON T.ContractNumber = A.ContractNumber 
	 LEFT OUTER JOIN [ICON_EntForms_Unit]U ON A.UnitNumber = U.UnitNumber AND A.ProductID = U.ProductID 
	 ...
	 LEFT OUTER JOIN [ICON_EntForms_TransferPayment]TP ON T.TransferNumber = TP.TransferNumber
